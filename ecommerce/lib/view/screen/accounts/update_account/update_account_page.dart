...
class UpdateAccountPage extends GetView<UpdateAccountController> {
  ...
  Widget birthDay() {
    return IZIInput(
      ...
      onChanged: (val) {
        controller.userRequest.born = IZIDate.parse(val).millisecondsSinceEpoch;
      },
      initValue: IZIValidate.nullOrEmpty(controller.currentUser)
          ? ''
          : IZIValidate.nullOrEmpty(controller.currentUser.born)
              ? ''
              : IZIDate.formatDate(DateTime.fromMillisecondsSinceEpoch(
                  controller.currentUser.born!)),
    );
  }
