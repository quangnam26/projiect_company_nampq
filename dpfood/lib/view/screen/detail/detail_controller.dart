...
class DetailController extends GetxController {
  ...
  void onInit() {
    ...
    if (arg != null) {
      ...
      strlist = [
        {"text": startAddress(), "icon": Icons.gps_fixed_outlined, "title": "Đi"},
        {"text": endAddress(), "icon": Icons.location_on_outlined, "title": "Đến"}
      ];
    }
  }
  ...
  ///
  String getTime() {
    final time = DateTime.fromMillisecondsSinceEpoch(transport!.timeStart!, isUtc: false);
  ...
  IconData getVehicleIcon(TransportResponse transport) {
    if (IZIValidate.nullOrEmpty(transport) || IZIValidate.nullOrEmpty(transport.idVehicle) || IZIValidate.nullOrEmpty(transport.idVehicle!.name)) {
  ...
  Future<void> onClickButton(BuildContext context) async {
    if (!IZIValidate.nullOrEmpty(transport) && !IZIValidate.nullOrEmpty(transport!.idUser)) {
      if (transport!.idUser!.id!.contains(idUser) && isFromHomePage == false) {
        showAnimatedDialog(
          context,
          DialogWidget(
            title: "Xác nhận",
            description: transport!.isActive == true ? "Khi bạn đồng ý, thông tin chuyến xe sẽ không còn hiển thị lên trang chủ" : "Khi bạn đồng ý, thông tin chuyến xe sẽ hiển thị lên trang chủ",
            onTapConfirm: () {
