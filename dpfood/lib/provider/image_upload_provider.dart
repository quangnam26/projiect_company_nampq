...
class ImageUploadProvider {
  ImageUploadRepository? regionRepo = GetIt.I.get<ImageUploadRepository>();
  ...
  Future<void> addImages({
    ...
    if (countSize > maxMBFilesSize * 1024 * 1024) {
      return onError(Exception(
          'Dung lượng ảnh không quá $maxMBFilesSize MB, vui lòng chọn file khác'));
    }
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      //Confirm image
      final ApiResponse apiResponseComfirm =
          await regionRepo!.confirmFilesOrImages(value);

      if (apiResponseComfirm.response.statusCode! >= 200 &&
          apiResponseComfirm.response.statusCode! <= 300) {
        final List<String> resultsConfirm = [];

        for (final item in results) {
          if (item.toString().contains('/tmp')) {
            resultsConfirm.add(item.toString().replaceAll('/tmp', '/image'));
          }
        }
  ...
  Future<void> addFiles({
    ...
    if (countSize > maxMBFilesSize * 1024 * 1024) {
      return onError(Exception(
          'Dung lượng file không quá $maxMBFilesSize MB, vui lòng chọn file khác'));
    }
