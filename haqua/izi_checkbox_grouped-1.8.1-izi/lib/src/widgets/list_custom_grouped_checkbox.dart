...
import 'package:izi_izi_checkbox_grouped/src/common/utilities.dart';
import 'package:izi_izi_checkbox_grouped/src/controller/custom_group_controller.dart';
...
class ListCustomGroupedCheckbox extends StatefulWidget {
  ...
  ListCustomGroupedCheckbox({
    ...
    Key? key,
        ...
        assert((groupTitles == null && (groupTitlesWidget != null && groupTitlesWidget.isNotEmpty)) || (groupTitlesWidget == null && (groupTitles != null && groupTitles.isNotEmpty))),
        super(key: key);

  static ListCustomGroupedCheckboxState? of(BuildContext context, {bool nullOk = false}) {
    final ListCustomGroupedCheckboxState? result = context.findAncestorStateOfType<ListCustomGroupedCheckboxState>();
    if (nullOk || result != null) return result;
    throw FlutterError.fromParts(<DiagnosticsNode>[ErrorSummary('ListGroupedCheckboxState.of() called with a context that does not contain an CustomGroupedCheckbox.'), ErrorDescription('No ListGroupedCheckboxState ancestor could be found starting from the context that was passed to CustomGroupedCheckbox.of().'), context.describeElement('The context used was')]);
  }

  @override
  ListCustomGroupedCheckboxState createState() => ListCustomGroupedCheckboxState();
}
...
class ListCustomGroupedCheckboxState extends State<ListCustomGroupedCheckbox> {
        ...
        (index) {
          ...
          if (isMultipleSelections[index]) {
            customGroupController = CustomGroupController.multiple(
              initSelectedItem: widget.controller.initSelectedValuesByGroup.isNotEmpty
                  ? widget.controller.initSelectedValuesByGroup.containsKey(index)
                      ? widget.controller.initSelectedValuesByGroup[index]!.toList()
                      : []
                  : [],
            )..listen((_) => _onSelected());
          } else {
            if (widget.controller.initSelectedValuesByGroup.containsKey(index) && widget.controller.initSelectedValuesByGroup[index]!.isNotEmpty) {
              customGroupController = CustomGroupController(
                initSelectedItem: widget.controller.initSelectedValuesByGroup[index]!.first,
              )..listen((_) => _onSelected());
            } else {
              customGroupController = CustomGroupController()..listen((_) => _onSelected());
            }
          }
  ...
  Future<List> getAllValues({bool byGroup = true}) async {
    final List resultList = List.empty(growable: true);
    final values = listControllers.map((e) {
  ...
  Future<Map<int, dynamic>> getMapValues() async {
    ...
    listControllers.asMap().forEach((key, controller) {
      resultList.putIfAbsent(key, () => controller.selectedItem);
    });
    return resultList;
  }
  ...
  Future<List> getListValuesByIndex(int index) async {
    assert(index < len);
    final List resultList = List.empty();
    resultList.addAll(listControllers[index].selectedItem as Iterable<dynamic>);
    return resultList;
  }
      ...
      itemBuilder: (ctx, index) {
        ...
        if (widget.children[index] is CustomGridIndexedWidgetBuilder) {
          return CustomGroupedCheckbox.grid(
            ...
            gridDelegate: (widget.children[index] as CustomGridIndexedWidgetBuilder).gridDelegate,
          );
        }
  ...
  Widget build(BuildContext context) {
    return title != null
        ? Container(
            ...
            child: Text(
              title!,
              style: titleGroupedTextStyle ?? Theme.of(context).textTheme.headline6,
            ),
          )
