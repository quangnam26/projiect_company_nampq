...
import 'package:izi_izi_checkbox_grouped/src/widgets/simple_grouped_chips.dart';
...
class ListGroupedCheckbox<T> extends StatefulWidget {
  ...
  ListGroupedCheckbox({
    ...
    Key? key,
        ...
        assert(controller.isMultipleSelectionPerGroup.isEmpty || controller.isMultipleSelectionPerGroup.length == titles.length),
        super(key: key);
...
class ListGroupedCheckboxState<T> extends State<ListGroupedCheckbox> {
    ...
    listControllers.addAll(
      List.generate(
        ...
        (index) => GroupController(
          initSelectedItem: widget.controller.initSelectedValues.isNotEmpty ? widget.controller.initSelectedValues[index] : [],
          isMultipleSelection: widget.controller.isMultipleSelectionPerGroup.isNotEmpty ? widget.controller.isMultipleSelectionPerGroup[index] : false,
        ),
      ),
    );
  }
  ...
  Future<List<T>> getAllValues() async {
    final List<T> resultList = List.empty(growable: true);
    final values = listControllers.map((e) => e.selectedItem).where((v) {
      if (v != null) {
    ...
    for (final v in values) {
      if (v is List) {
        ...
        if (v != null) resultList.add(v as T);
      }
    }
  ...
  Future<List<T>> getValuesByIndex(int index) async {
    assert(index < len);
    final List<T> resultList = List.empty();
    resultList.addAll(listControllers[index].selectedItem as Iterable<T>);
    return resultList;
  }
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (ctx, index) {
        if (widget.mapItemGroupedType != null && widget.mapItemGroupedType!.isNotEmpty) {
          if (widget.mapItemGroupedType!.containsKey(index)) {
            ...
            } else if (widget.mapItemGroupedType![index] == GroupedType.Switch) {
              return Column(
                children: [
                  ListTile(
                    title: Text(
        ...
        return SimpleGroupedCheckbox<T>(
          ...
          itemsTitle: widget.titles[index],
          values: widget.values[index] as List<T>,
          disableItems: widget.disabledValues.isNotEmpty ? widget.disabledValues[index] as List<String> : [],
