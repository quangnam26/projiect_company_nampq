...
void main() {
  testWidgets("test CustomGroupedCheckbox ", (tester) async {
    final CustomGroupController controller = CustomGroupController();
    ...
    await tester.tap(find.byKey(const Key("1")), warnIfMissed: false);
    await tester.pumpAndSettle();
    expect(controller.selectedItem, 1);

    await tester.tap(find.byKey(const Key("2")), warnIfMissed: false);
    await tester.pump();
    expect(controller.selectedItem, 2);
  });
  testWidgets("test multiple selection CustomGroupedCheckbox ", (tester) async {
    final CustomGroupController controller = CustomGroupController(isMultipleSelection: true);
  ...
  testWidgets("test disable/Enable selection CustomGroupedCheckbox ", (tester) async {
    final CustomGroupController controller = CustomGroupController(isMultipleSelection: true);
