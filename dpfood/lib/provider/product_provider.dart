...
class ProductProvider {
  ...
  Future<void> all({
    ...
    if (apiResponse.response.statusCode! >= 200 && apiResponse.response.statusCode! <= 300) {
      ...
      onSuccess(results.map((e) => ProductResponse.fromJson(e as Map<String, dynamic>)).toList());
    } else {
  ...
  Future<void> search({
    ...
    final ApiResponse apiResponse = await repository!.search(page, limit, filter);
    if (apiResponse.response.statusCode == null) {
      return onError(apiResponse.error);
    }
    if (apiResponse.response.statusCode! >= 200 && apiResponse.response.statusCode! <= 300) {
      // call back data success
      final results = apiResponse.response.data.toString() != '[]' ? apiResponse.response.data as List<dynamic> : [];
      onSuccess(results.map((e) => ProductResponse.fromJson(e as Map<String, dynamic>)).toList());
    } else {
      onError(apiResponse.error);
    }
  ...
  Future<void> find({
    required String id,
    required Function(ProductResponse category) onSuccess,
    required Function(dynamic error) onError,
  }) async {
