...
class AddressRepository {
  ...
  Future<ApiResponse> add(AddressRequest data) async {
    try {
      final response = await dioClient!.post('/addresses', data: data.toJson());
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> update(AddressRequest data,String id) async {
    try {
      final response = await dioClient!.put('/addresses/$id', data: data.toJson());
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> paginate(int page, int limit, String filter) async {
    try {
      String uri = '/addresses/paginate?page=$page&limit=$limit'.toString();

      // add condition filter
      if (filter != '') {
        uri = '/addresses/paginate?page=$page&limit=$limit$filter';
      }
