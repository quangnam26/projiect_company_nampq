...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> signinWithSocial({
    ...
    final ApiResponse apiResponse =
        await authRepository!.signinWithSocial(request);
    if (apiResponse.response.statusCode == null) {
      onError(apiResponse.error);
    }
    if (apiResponse.response.statusCode! >= 200 &&
  ...
  Future<void> postLogOut({
    required AuthRequest data,
    required Function(AuthRequest user) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    final ApiResponse apiResponse = await authRepository!.logoutSignOut(data);
    if (apiResponse.response.statusCode == null) {
      return onError(apiResponse.error);
    }
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      // call back data success
      final results = apiResponse.response.data as dynamic;
      onSuccess(AuthRequest.fromMap(results as Map<String, dynamic>));
    } else {
