...
abstract class StateGroup<K, T extends StatefulWidget> extends State<T>
  ...
  void init({
    ...
    itemsTitle.asMap().forEach((key, title) {
      ...
      if (multiSelection && preSelection.isNotEmpty) {
  ...
  void disabledItemsByValues(List<dynamic> itemsValues) {
    ...
    final items = _recuperateTitleFromValues(itemsValues.cast<K>());
  ...
  List<String?> _recuperateTitleFromValues(List<K> itemsValues) {
    return itemsValues.map((e) {
      final indexOfItem = values.indexOf(e);
      return items[indexOfItem].title;
    }).toList();
  }

  void enableAll() {
    for (final notifierItem in notifierItems) {
      final index = notifierItems.indexOf(notifierItem);
      final Item item = notifierItem.value.copy();
  ...
  void disableAll() {
    for (final notifierItem in notifierItems) {
      final index = notifierItems.indexOf(notifierItem);
      final Item item = notifierItem.value.copy(isDisabled: true);
      notifierItems[index].value = item;
    }
  }
  ...
  void _itemStatus(List<String?> items, bool isDisabled) {
    notifierItems
      ...
      final index = notifierItems.indexOf(notifierItem);
      final Item item = notifierItem.value.copy(isDisabled: isDisabled);
      notifierItems[index].value = item;
    });
  }
