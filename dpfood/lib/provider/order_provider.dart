...
class OrderProvider {
  ...
  ///
  Future<void> all({
    required Function(List<OrderResponse> orders) onSuccess,
  ...
  Future<void> add({
    ...
    required Function(OrderRequest order) onSuccess,
    required Function(dynamic error) onError,
  }) async {
  ...
  /// Get paginate Orders "page": 1, "limit": 10
  ///
  Future<void> paginateHistory({
    required int page,
    required int limit,
    required String filter,
    required Function(List<OrderHistoryResponse> orders) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    final ApiResponse apiResponse =
        await repository!.paginate(page, limit, filter);
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      final results = apiResponse.response.data.toString() != '[]'
          ...
          : [];
      onSuccess(results
          .map((e) => OrderHistoryResponse.fromJson(e as Map<String, dynamic>))
          .toList());
    } else {
      onError(apiResponse.error);
    }
  ...
  Future<void> find({
    required String id,
    required Function(OrderResponse order) onSuccess,
    required Function(dynamic error) onError,
  }) async {
  ...
  Future<void> countTheProduct({
    required String id,
    required Function(int count) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    final ApiResponse apiResponse = await repository!.countTheProduct(id);
    if (apiResponse.response.statusCode == null) {
      return onError(apiResponse.error);
    }
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      // call back data success
      final results = apiResponse.response.data as dynamic;
      onSuccess(IZINumber.parseInt(results['count']));
    } else {
      onError(apiResponse.error);
    }
  }

  Future<void> countTheProductsOfStore({
    required String id,
    required Function(int count) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    final ApiResponse apiResponse =
        await repository!.countTheProductsOfStore(id);
    if (apiResponse.response.statusCode == null) {
      return onError(apiResponse.error);
    }
    if (apiResponse.response.statusCode! >= 200 &&
