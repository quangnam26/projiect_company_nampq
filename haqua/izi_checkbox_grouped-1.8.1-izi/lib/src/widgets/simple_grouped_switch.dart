...
class SimpleGroupedSwitch<T> extends StatefulWidget {
  ...
  SimpleGroupedSwitch({
    ...
    this.onItemSelected,
        ...
        assert(
          disableItems.isEmpty || disableItems.takeWhile((c) => values.contains(c)).isNotEmpty,
          "you cannot disable item doesn't exist",
        ),
        super(key: key);

  static SimpleGroupedSwitchState? of<T>(BuildContext context, {bool nullOk = false}) {
    final SimpleGroupedSwitchState<T>? result = context.findAncestorStateOfType<SimpleGroupedSwitchState<T>>();
    if (nullOk || result != null) return result;
    throw FlutterError.fromParts(<DiagnosticsNode>[ErrorSummary('SimpleGroupedSwitch.of() called with a context that does not contain an SimpleGroupedSwitch.'), ErrorDescription('No SimpleGroupedSwitch ancestor could be found starting from the context that was passed to SimpleGroupedSwitch.of().'), context.describeElement('The context used was')]);
  }
...
class SimpleGroupedSwitchState<T> extends StateGroup<T, SimpleGroupedSwitch> {
  ...
  void initState() {
    ...
    init(
      ...
      disableItems: widget.itemsTitle.where((element) => widget.disableItems.contains(widget.values[widget.itemsTitle.indexOf(element)])).toList(),
        ...
        return ValueListenableBuilder<Item>(
          ...
          builder: (ctx, item, _) {
            return _SwitchListItem(
              ...
              textStyle: widget.groupStyle?.itemTitleStyle?.copyWith(color: item.checked! ? widget.groupStyle?.activeColor : widget.groupStyle?.itemTitleStyle?.color),
            );
          },
        );
  ...
  void changeSelection(int index, value) {
    final Item item = notifierItems[index].value.copy();
    if (widget.controller.isMultipleSelection) {
      ...
      if (!item.checked! && value as bool) {
        notifierItems[index].value = item.copy(checked: value);
        if (value) {
          if (widget.values.indexOf(selectedValue.value) != index) {
            //_items[index].checked = false;
            if (selectedValue.value != null) {
              final indexPreviousItem = widget.values.indexOf(selectedValue.value);
              final previousItem = notifierItems[indexPreviousItem].value;
              notifierItems[indexPreviousItem].value = previousItem.copy(checked: false);
            }
