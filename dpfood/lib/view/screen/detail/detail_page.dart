...
import '../../../base_widget/preview_image.dart';
...
class DetailGoCommonPage extends GetView<DetailController> {
            ...
            children: [
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: ClipRRect(
                        ...
                        child: IZIImage(
                          IZIValidate.nullOrEmpty(controller.transport!.idUser) ? '' : controller.transport!.idUser!.avatar ?? '',
                        ),
                      ),
                    ),
                    Container(
                        ...
                        children: [
                          ...
                          IZIText(
                            text: IZIValidate.nullOrEmpty(controller.transport)
                                ...
                                : IZIValidate.nullOrEmpty(controller.transport!.idUser)
                                    ? ''
                                    : IZIValidate.nullOrEmpty(controller.transport!.idUser!.phone)
                                        ? ''
                                        : controller.transport!.idUser!.phone ?? '',
                            style: TextStyle(
              ...
              Container(
                ...
                child: Column(
                  children: [
                    infoContent(
                      value: controller.getLoaiHinh(controller.transport!.transportType!),
                      icon: Icons.grid_view_outlined,
                    ),
                    if (!IZIValidate.nullOrEmpty(controller.transport!.idVehicle))
                      infoContent(
                        value: IZIValidate.nullOrEmpty(controller.transport)
                            ? ''
                            : IZIValidate.nullOrEmpty(controller.transport!.idVehicle)
                                ? ''
                                : controller.transport!.idVehicle!.name ?? '',
                        icon: controller.getVehicleIcon(controller.transport!),
                      ),
                    infoContent(
                      value: controller.getValueTransportType(controller.transport!.transportType!),
                      icon: controller.getLoaiHinhIcon(controller.transport!.transportType!),
                    ),
                    infoContent(
                      value: IZIValidate.nullOrEmpty(controller.transport)
                          ? ''
                          : IZIValidate.nullOrEmpty(controller.transport!.timeStart)
                              ? ''
                              : controller.getTime(),
                    ...
                    infoContent(
                      value: IZIValidate.nullOrEmpty(controller.transport)
                          ? ''
                          : IZIValidate.nullOrEmpty(controller.transport!.dateStart)
                              ? ''
                              : IZIDate.formatDate(DateTime.fromMillisecondsSinceEpoch(controller.transport!.dateStart!)),
                      icon: Icons.date_range,
                    ),
                    if (!IZIValidate.nullOrEmpty(controller.transport!.description))
                      infoContent(
                        value: IZIValidate.nullOrEmpty(controller.transport)
                            ? ''
                            : IZIValidate.nullOrEmpty(controller.transport!.description)
                                ? ''
              ...
              if (!IZIValidate.nullOrEmpty(controller.transport!.images))
                Column(
                  children: [
                    ...List.generate(
                      controller.transport!.images!.length,
                      (index) {
                        return GestureDetector(
                          onTap: () {
                            Get.to((){
                              return ReviewImagePage(
                                image: controller.transport!.images![index],
                              );
                            });
                          },
                          child: Container(
                            ...
                            child: ClipRRect(
                              ...
                              child: IZIImage(
                                IZIValidate.nullOrEmpty(controller.transport!.images) ? '' : controller.transport!.images![index].toString(),
                              ),
                            ),
                          ),
                        );
        ...
        child: IZIButton(
          ...
          colorBG: IZIValidate.nullOrEmpty(controller.transport!.idUser)
              ...
              : controller.transport!.idUser!.id == controller.idUser && controller.isFromHomePage == false
                  ? controller.transport!.isActive == true
                      ? ColorResources.RED
  ...
  Widget timeLine() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...List.generate(controller.strlist.length, (index) {
            return Container(
              margin: EdgeInsets.only(
              ...
              child: RichText(
                text: TextSpan(
                    text: '${controller.strlist[index]['title'].toString()}: ',
                    style: TextStyle(
                      fontSize: IZIDimensions.FONT_SIZE_H6,
                    ...
                    children: [
                      TextSpan(
                        text: controller.strlist[index]['text'].toString(),
                        style: TextStyle(
                          fontSize: IZIDimensions.FONT_SIZE_H6,
            ...
            //   children: [
            //     Text(
            //       '${controller.strlist[index]['title'].toString()}:',
            //       style: TextStyle(
            //         fontSize: IZIDimensions.FONT_SIZE_H6,
            //         fontWeight: FontWeight.w600,
            //         color: ColorResources.NEUTRALS_3,
    ...
    //       return Text(controller.strlist[index]['title'].toString());
    //       // return DotIndicator(
  ...
  Widget infoContent({required String value, required IconData icon, CrossAxisAlignment? alignment}) {
    return Container(
      margin: EdgeInsets.only(
        bottom: IZIDimensions.SPACE_SIZE_2X,
      ),
